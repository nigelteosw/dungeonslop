...
8. **Needs a real UI kit — fantasy-styled, in the spirit of *Tactical Breach Wizards*.** Every HUD element right now (`CardHand`, `LobbyScreen`, the corner status text, the ROOM CLEAR/DEFEATED banner) is plain dark-slate/system-ui default styling with no art direction. TBW's UI is the reference point: bold chunky-bordered panels, punchy graphic-novel typography, strong flat color blocking, playful iconography — translated here into a fantasy idiom (runes/parchment/heraldry motifs instead of TBW's modern-tactical look) rather than a generic dark dashboard. This is a cohesive pass across all 2D overlay chrome at once (a shared theme: colors, borders, font, card frame, button style), not a one-off per component — worth doing as its own design pass once the interaction/feel items above are in, so the kit is built for a UI that's already settled.
