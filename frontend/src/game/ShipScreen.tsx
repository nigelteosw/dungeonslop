...
export function ShipScreen({ state, myCrewId, mySessionId, players, error, onCommand, onVote }: Props) {
  ...
  const crisisItems: CrisisItem[] = [
    ...
    ...(incapacitatedCrew.length > 0 ? [{ id: 'crew', tone: 'critical' as const, title: `${incapacitatedCrew.length} crew member${incapacitatedCrew.length === 1 ? '' : 's'} down`, detail: incapacitatedCrew.map((crew) => `${crew.name} (${Math.max(0, Math.ceil(crew.bleedoutTicks * 0.4))}s to revive)`).join(' · ') }] : []),
