...
export function ShipScreen({ state, myCrewId, mySessionId, players, error, onCommand, onVote }: Props) {
  ...
  useEffect(()=>{const handler=(event:KeyboardEvent)=>{if(state.status!=='encounter'||event.target instanceof HTMLInputElement||event.target instanceof HTMLTextAreaElement)return;const vectors:Record<string,[-1|0|1,-1|0|1]>={w:[0,-1],a:[-1,0],s:[0,1],d:[1,0]};const vector=vectors[event.key.toLowerCase()];if(vector){event.preventDefault();const now=Date.now();if(now-lastMoveAt.current<220)return;lastMoveAt.current=now;onCommand({kind:'moveVector',crewId:me.id,dx:vector[0],dy:vector[1]});}else if(event.key.toLowerCase()==='f'){event.preventDefault();performContextAction();}};window.addEventListener('keydown',handler);return()=>window.removeEventListener('keydown',handler);});
